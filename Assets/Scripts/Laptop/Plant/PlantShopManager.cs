...
{   
    ...
    void OnEnable() {
        ...
        checkoutBtn.onClick.AddListener(delegate { Checkout(true); });
        checkoutNoUpgradeBtn.onClick.AddListener(delegate { Checkout(false); });
    }

    private void Checkout(bool upgrade) {
        // not setting these directly into PlayerChoices yet because the user hasn't confirmed buying
        plantPremium = upgrade;
        ribbonColor = ribbonInput.options[ribbonInput.value].text;
        
        checkoutPage.gameObject.SetActive(true);
